...
public class SourceFileAssert extends AbstractAssert<SourceFileAssert, SourceFile> {
	...
	public MethodAssert hasMethodNamed(String name) {
				...
				if (method != null) {
					throw Failures.instance().failure(info,
							new BasicErrorMessageFactory(
									String.format("%nExpecting actual:%n  %s%nto contain unique method:%n  %s%n",
		...
		if (method == null) {
			throw Failures.instance().failure(info, new BasicErrorMessageFactory(String
					.format("%nExpecting actual:%n  %s%nto contain method:%n  %s%n", this.actual.getContent(), name)));
		}
	...
	public MethodAssert hasMethod(String name, Class<?>... parameters) {
		...
		JavaMethod method = null;
		for (JavaMethod candidate : javaClass.getMethods()) {
			if (candidate.getName().equals(name) && hasParameters(candidate, parameters)) {
				if (method != null) {
					throw Failures.instance().failure(info,
							new BasicErrorMessageFactory(
									String.format("%nExpecting actual:%n  %s%nto contain unique method:%n  %s%n",
		...
		if (method == null) {
			...
			throw Failures.instance().failure(info,
					new BasicErrorMessageFactory(String.format("%nExpecting actual:%n  %s%nto contain method:%n  %s%n",
	...
	private boolean hasParameters(JavaMethod method, Class<?>[] requiredParameters) {
		...
		for (int i = 0; i < requiredParameters.length; i++) {
			if (!requiredParameters[i].getName().equals(parameters.get(i).getFullyQualifiedName())) {
				return false;
			}
		}
		return true;
	...
	private String getMethodDescription(String name, Class<?>... parameters) {
		return name + "(" + Arrays.stream(parameters).map(Class::getName).collect(Collectors.joining(", ")) + ")";
	}

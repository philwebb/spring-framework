...
/**
 * Strategy interface used to select beans from a {@link BeanRepository}.
 *
 * @author Phillip Webb
 * @since 6.0.0
...
public interface BeanSelector<T> {
	 ...
	 */
	boolean test(BeanRegistration<?> registration);

	/**
	 * Return the names of the beans that should be tested for selection.
	 * Returning a value from this method allows {@link BeanRepository}
	 ...
	 */
	static <T> BeanSelector<T> havingType(Class<? extends T> type) {
	 ...
	 */
	static <T> BeanSelector<T> havingName(String name) {
		return null;
	}

 ...
 * }</pre>
 * <p>
 * Will also generate reflection based injection and register hints if the member is not
 * visible.
 *
...
public class InjectionCodeGenerator {
	...
	public CodeBlock generateInjectionCode(Member member, String instanceVariable, CodeBlock resourceToInject) {
	...
	private CodeBlock generateFieldInjectionCode(Field field, String instanceVariable, CodeBlock resourceToInject) {
		...
		if (visibility == AccessVisibility.PRIVATE || visibility == AccessVisibility.PROTECTED) {
			...
			builder.addStatement("$T field = $T.findField($T.class, $S)", Field.class, ReflectionUtils.class,
					field.getDeclaringClass(), field.getName());
			builder.addStatement("$T.makeAccessible($L)", ReflectionUtils.class, "field");
			builder.addStatement("$T.setField($L, $L, $L)", ReflectionUtils.class, "field", instanceVariable,
					resourceToInject);
		}
		else {
			builder.addStatement("$L.$L = $L", instanceVariable, field.getName(), resourceToInject);
		}
	...
	private CodeBlock generateMethodInjectionCode(Method method, String instanceVariable, CodeBlock resourceToInject) {
		...
		if (visibility == AccessVisibility.PRIVATE || visibility == AccessVisibility.PROTECTED) {
			...
			builder.addStatement("$T method = $T.findMethod($T.class, $S, $T.class)", Method.class,
					ReflectionUtils.class, method.getDeclaringClass(), method.getName(), method.getParameterTypes()[0]);
			builder.addStatement("$T.makeAccessible($L)", ReflectionUtils.class, "method");
			builder.addStatement("$T.invokeMethod($L, $L, $L)", ReflectionUtils.class, "method", instanceVariable,
					resourceToInject);
		}
		else {
			builder.addStatement("$L.$L($L)", instanceVariable, method.getName(), resourceToInject);
		}
		return builder.build();
	}

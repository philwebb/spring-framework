...
public final class BeanSelector<T> {
	...
	private BeanSelector(@Nullable PrimarySelectorType primarySelectorType,
			@Nullable Object primarySelector, boolean onlyUsingPrimarySelector,
			Predicate<FunctionalBeanDefinition<?>> predicate,
	...
	/**
	 * Return {@code true} if only the primary selector is being used to test
	 * beans.
	 ...
	 * {@link FunctionalBeanDefinition bean definitions} with the specified
	 * qualifier.
	...
	public BeanSelector<T> withFilter(String description,
			Predicate<FunctionalBeanDefinition<?>> filter) {
	...
	public BeanSelector<T> withFilter(Supplier<String> description,
		...
		return new BeanSelector<>(this.primarySelectorType, this.primarySelector, false,
				this.predicate.and(filter),
				merge(this.flags, this.description, description), flags);
	}

	/**
	 * Test the given {@link FunctionalBeanDefinition} to see if it should be
	 * selected.
	...
	public static <T> BeanSelector<T> all() {
		return new BeanSelector<>(null, null, false, (descriptor) -> true,
				() -> "All beans", EnumSet.of(Flag.MERGE_DESCRIPTION_WITHOUT_AND));
	}
	...
	public static <T> BeanSelector<T> byName(String name) {
		return new BeanSelector<T>(PrimarySelectorType.NAME, name, true,
	...
	public static <T> BeanSelector<T> byType(Class<? extends T> type) {
		return new BeanSelector<T>(PrimarySelectorType.TYPE, type, true,
	...
	public static <T> BeanSelector<T> byAnnotation(
		...
		return new BeanSelector<T>(PrimarySelectorType.ANNOTATION_TYPE, annotationType,
				true, (descriptor) -> descriptor.hasAnnotation(annotationType),

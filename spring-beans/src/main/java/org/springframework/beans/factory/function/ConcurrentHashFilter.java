...
/**
 * Memory efficient filter used to quickly limit candidates based on the hash codes of
 * attributes extracted from the element. For example, a collection of Strings could be
 * filtered based on their letters to allow fast retrieval of elements that are likely to
 * match an {@code String.contains(char)} check.
 *
 * @author Phillip Webb
...
class ConcurrentHashFilter<E, A> {
	...
	ConcurrentHashFilter(HashCodesExtractor<E, A> hashCodesExtractor) {
		this(hashCodesExtractor, DEFAULT_COLLISION_THRESHOLD, DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR,
	 ...
	 * Create a new {@link ConcurrentHashFilter} instance.
	 * @param hashCodesExtractor the hash codes extractor to apply
	 * @param collisionThreshold the maximum number of elements that can be stored for a
	 * given attribute hash code. Once exceeded, {@link #findCandidates(Object)} will not
	 * return a result and a full scan will be required.
	 */
	ConcurrentHashFilter(HashCodesExtractor<E, A> hashCodesExtractor, int collisionThreshold) {
		this(hashCodesExtractor, collisionThreshold, DEFAULT_INITIAL_CAPACITY, DEFAULT_LOAD_FACTOR,
				DEFAULT_CONCURRENCY_LEVEL);
	}
	 ...
	 * @param concurrencyLevel the expected number of threads that will concurrently use
	 * the filter
	 */
	@SuppressWarnings("unchecked")
	ConcurrentHashFilter(HashCodesExtractor<E, A> hashCodesExtractor, int collisionThreshold, int initialCapacity,
			float loadFactor, int concurrencyLevel) {
		Assert.notNull(hashCodesExtractor, "HashCodesExtractor must not be null");
		Assert.isTrue(collisionThreshold >= 0, "Collision threshold must not be negative");
	...
	/**
	 * Add the given element to the filter, applying the {@link HashCodesExtractor} to
	...
	/**
	 * Add the given element to the filter, applying the {@link HashCodesExtractor} to
	 * remove any attribute associations.
	...
	/**
	 * Find candidate elements that are likely to have the specified attribute based on
	 * its hash code.
	 * @param attributeHashCode the hashcode of the attribute that the candidates must
	 * have
	 * @return the candidates or {@code null} if a full scan is required
	 */
	@Nullable
	...
	/**
	 * Get the hash for a given object, apply an additional Wang/Jenkins hash function to
	 * reduce collisions.
	 * @param hashCode the source hash code
	...
	/**
	 * A segment used to divide the hash table to improve multi-threaded performance.
	 */
	...
	class Segment extends ReentrantLock {
		...
		/**
		 * The entry table that contains both the linear probing hash table and the
		 * values. The first entry in the array always an {@code int[]} hash table. The
		 * remaining entries are the values which may be a single associated value, a
		 * {@link Elements} container or {@link #COLLISION_THRESHOLD_EXCEEDED}.
		 */
	...
	/**
	 * Container used when more than one element is associated with an attribute hash.
	 * @param <E> the element type
	 */
	private static class Elements<E> implements Candidates<E> {
		...
		public Elements(Object initial, E additional) {
			this.elements = (Object[]) Array.newInstance(Object.class, DEFAULT_INITIAL_CAPACITY);
			this.elements[0] = initial;
			this.elements[1] = additional;
			this.size = 2;
			this.end = 2;
		}

		/**
		 * Add a new value if it isn't already in the collection. This method is not
		 * thread-safe and must only be called when the segment is locked.
		 * @param element the element to add
		 */
		void add(Object element) {
			...
			for (int i = 0; i < elements.length; i++) {
				...
				containsDeleted = containsDeleted || elements[i] == DELETED;
			}
			// We must create a new array whenever elements are moved so that we
			// don't break the iterator.
			int numberDeleted = 0;
			if (containsDeleted) {
				elements = (Object[]) Array.newInstance(Object.class, elements.length);
				int i = 0;
				for (Object existing : this.elements) {
					if (existing != DELETED) {
						elements[i] = existing;
						numberDeleted++;
						i++;
				...
				elements = (Object[]) Array.newInstance(Object.class, elements.length + RESIZE_INCREASE);
				System.arraycopy(this.elements, 0, elements, 0, this.elements.length);
				elements[this.elements.length] = element;
			}
			this.elements = elements;
			this.end -= numberDeleted;
			this.size++;
		}

		/**
		 * Remove an existing value if it's in the collection. This method is not
		 * thread-safe and must only be called when the segment is locked.
		...
		public Iterator<E> iterator() {
			return new ElementsIterator<E>(this.elements);
		}

		@Override
		@SuppressWarnings("unchecked")
		public Stream<E> stream() {
			return (Stream<E>) Arrays.stream(this.elements, 0, this.end).filter(
					element -> element != DELETED && element != null);
		}

	}

	/**
	 * Iterator for {@link Elements}. This iterator requires that elements in the backing
	 * array to not change position.
	 * @param <E> the element type
	 */
	...
	/**
	 * Container used to return a {@link Candidates} result for a single element.
	 * @param <E> the element type
	 */
	...
	interface HashCodesExtractor<E, A> {
		...
		/**
		 * Factory method that can be used to create a {@link HashCodesExtractor} for a
		 * set of pre-computed hash codes.
		 * @param <E> the element type
		 * @param <A> the attribute type
		 * @param hashCodes the pre-computed hash codes
		 * @return a {@link HashCodesExtractor} instance that provides the pre-computed
		 * hash codes
		 */
		static <E, A> HashCodesExtractor<E, A> preComputed(int... hashCodes) {
			return (value, consumer) -> consumer.acceptHashCodes(hashCodes);
		}
	...
	interface HashCodeConsumer<A> {
		...
		/**
		 * Accept the hash code of the given attribute generated by a given function.
		 * @param attribute the attribute to accept
		 * @param hashFunction the function used to generate the hash code
		 */
		default void accept(A attribute, HashCodeFunction<A> hashFunction) {
			acceptHashCode(hashFunction.apply(attribute));
		}
	...
	/**
	 * An {@link Iterable} collection of candidates that are likely to have a specific
	 * attribute.
	 * @param <E> the element type
	 */

 ...
 */
@FunctionalInterface
public interface FunctionalBeanInstanceSupplier<T> {

	/**
	 * Return the full-wired bean instance with dependencies supplied via the
	 * given {@link FunctionalBeanRegistry}.
	 * @param repository the registry used to obtain dependencies
	 * @return the supplied bean instance
	 */
	T get(XBeanRepository repository) throws Exception;

	/**
	 * Create a new {@link FunctionalBeanInstanceSupplier} where the instance is supplied
	 ...
	 */
	static <T> FunctionalBeanInstanceSupplier<T> via(Supplier<T> supplier) {

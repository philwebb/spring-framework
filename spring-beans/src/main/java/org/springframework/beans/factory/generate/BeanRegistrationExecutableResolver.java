...
class BeanRegistrationExecutableResolver {
	...
	private ResolvableType determineParameterValueType(ValueHolder valueHolder) {
		...
		if (value instanceof BeanReference) {
			return ResolvableType.forClass(this.beanFactory.getType(((BeanReference) value).getBeanName(), false));
		}
	...
	/**
	 * Return a {@link Predicate} for a parameter type that checks if its target value is
	 * a {@link Class} and the value type is a {@link String}. This is a regular use cases
	 * where a {@link Class} is defined in the bean definition as an FQN.

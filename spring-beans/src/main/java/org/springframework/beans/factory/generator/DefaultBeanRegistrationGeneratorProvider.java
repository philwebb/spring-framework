...
/**
 * Default {@link BeanRegistrationGeneratorProvider} implementation.
 *
 * @author Stephane Nicoll
 * @since 6.0
 */
public final class DefaultBeanRegistrationGeneratorProvider implements BeanRegistrationGeneratorProvider, BeanInstantiationGeneratorProvider {

	private final ExecutableProvider executableProvider;

	public DefaultBeanRegistrationGeneratorProvider(DefaultListableBeanFactory beanFactory) {
		this.executableProvider = new ExecutableProvider(beanFactory);
	}

	@Override
	public BeanRegistrationGenerator getBeanRegistrationGenerator(String beanName, BeanDefinition beanDefinition) {
		return new DefaultBeanRegistrationGenerator(beanName, beanDefinition,
				getBeanInstantiationGenerator(beanName, beanDefinition));
	}

	@Override
	public BeanInstantiationGenerator getBeanInstantiationGenerator(String beanName, BeanDefinition beanDefinition) {
		return new DefaultBeanInstantiationGenerator(determineExecutable(beanDefinition),
				determineBeanInstanceContributors(beanName, beanDefinition));
	}

	private Executable determineExecutable(BeanDefinition beanDefinition) {
	...
	private List<BeanInstantiationContributor> determineBeanInstanceContributors(String beanName, BeanDefinition beanDefinition) {
		// TODO: find a way to invoke BeanInstanceAotPostProcessor

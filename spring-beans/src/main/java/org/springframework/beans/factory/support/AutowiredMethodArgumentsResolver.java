...
/**
 * Resolver used to support the autowiring of methods. Typically used in AOT-processed
 ...
 * <p>
 * When resolving arguments in a native image, the {@link Method} being used must be
 * marked with an {@link ExecutableMode#INTROSPECT introspection} hint so that field
 * annotations can be read. Full {@link ExecutableMode#INVOKE invocation} hints are only
 * required if the {@link #resolveAndInvoke(RegisteredBean, Object)} method of this class
 * is being used (typically to support private methods).
 *
...
public final class AutowiredMethodArgumentsResolver extends AutowiredElementResolver {
	...
	/**
	 * Create a new {@link AutowiredMethodArgumentsResolver} for the specified method
	...
	/**
	 * Resolve the method arguments for the specified registered bean and provide it to
	 * the given action.
	...
	/**
	 * Resolve the method arguments for the specified registered bean and invoke the
	...
	private Object[] resolveValue(RegisteredBean registeredBean, Method method) {
		...
		for (int i = 0; i < argumentCount; i++) {
			...
			try {
				Object argument = autowireCapableBeanFactory.resolveDependency(descriptor, beanName, autowiredBeanNames,
						typeConverter);

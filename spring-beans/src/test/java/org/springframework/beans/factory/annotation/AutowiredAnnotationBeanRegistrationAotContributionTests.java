...
class AutowiredAnnotationBeanRegistrationAotContributionTests {
	...
	void contributeWhenPrivateFieldInjectionInjectsUsingReflection() {
		...
		testCompiledResult(registeredBean, (postProcessor, compiled) -> {
			...
			postProcessor.postProcessInstance(registeredBean, instance);
			assertThat(instance).extracting("environment").isSameAs(environment);
			assertThat(compiled.getSourceFileFromPackage(getClass().getPackageName())).contains("resolveAndSet(");
		});
	}
	...
	void contributeWhenPrivateMethodInjectionInjectsUsingReflection() {
		...
		testCompiledResult(registeredBean, (postProcessor, compiled) -> {
			...
			postProcessor.postProcessInstance(registeredBean, instance);
			assertThat(instance).extracting("environment").isSameAs(environment);
			assertThat(compiled.getSourceFileFromPackage(getClass().getPackageName())).contains("resolveAndInvoke(");
		});
	}
	...
	private void testCompiledResult(RegisteredBean registeredBean,
			BiConsumer<InstancePostProcessor<Object>, Compiled> result) {
		...
		TestCompiler.forSystem().withSources(sourceFiles)
				.compile(compiled -> result.accept(compiled.getInstance(InstancePostProcessor.class), compiled));
	}

	private JavaFile createJavaFile(Class<?> target) {
		MethodReference methodReference = this.beanRegistrationCode.instancePostProcessors.get(0);
		TypeSpec.Builder builder = TypeSpec.classBuilder("TestPostProcessor");
		builder.addModifiers(Modifier.PUBLIC);
		builder.addSuperinterface(ParameterizedTypeName.get(InstancePostProcessor.class, target));
		builder.addMethod(MethodSpec.methodBuilder("postProcessInstance").addModifiers(Modifier.PUBLIC)
				.addParameter(RegisteredBean.class, "registeredBean").addParameter(target, "instance").returns(target)
				.addStatement("return $L",
						methodReference.toInvokeCodeBlock(CodeBlock.of("registeredBean"), CodeBlock.of("instance")))
				.build());

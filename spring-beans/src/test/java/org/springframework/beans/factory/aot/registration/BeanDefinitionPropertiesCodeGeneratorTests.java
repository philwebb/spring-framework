...
class BeanDefinitionPropertiesCodeGeneratorTests {
	...
	void setInitMethodWhenSingleInitMethod() {
		...
		testCompiledResult((actual, compiled) -> assertThat(actual.getInitMethodNames()).containsExactly("i1"));
	...
	void setInitMethodWhenMultipleInitMethods() {
		...
		testCompiledResult((actual, compiled) -> assertThat(actual.getInitMethodNames()).containsExactly("i1", "i2"));
	...
	void setDestroyMethodWhenDestroyInitMethod() {
		...
		testCompiledResult((actual, compiled) -> assertThat(actual.getDestroyMethodNames()).containsExactly("d1"));
	...
	private void assertHasMethodInvokeHints(String... methodNames) {
		assertThat(hints.reflection().getTypeHint(InitDestroyBean.class)).satisfies(typeHint -> {

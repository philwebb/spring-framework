...
class DefaultBeanRegistrationGeneratorTests {
	...
	void generateWithBeanDefinitionHavingPropertyAsBeanDefinition() {
		...
		CodeSnippet registration = beanRegistration(beanFactory, beanDefinition, createGeneratedTypeContext());
		assertThat(registration.getSnippet()).isEqualTo("""
	...
	private CodeSnippet beanRegistration(DefaultListableBeanFactory beanFactory, BeanDefinition beanDefinition,
			GeneratedTypeContext context) {
		DefaultBeanRegistrationGenerator generator = new DefaultBeanRegistrationGenerator("test", beanDefinition,
				getBeanInstanceGeneratorProvider(beanFactory));
	...
	private BeanInstantiationGeneratorProvider getBeanInstanceGeneratorProvider(
			DefaultListableBeanFactory beanFactory) {
		return new DefaultBeanRegistrationGeneratorProvider(beanFactory);
	}

...
class ApplicationContextInitializationCodeGenerator implements BeanFactoryInitializationCode {
	...
	JavaFile generateJavaFile(ClassName className) {
		...
		builder.addJavadoc("{@link $T} to restore an application context based on previous AOT processing.",
	...
	private MethodSpec generateInitializeMethod() {
		...
		builder.addModifiers(Modifier.PUBLIC);
		builder.addParameter(GenericApplicationContext.class, APPLICATION_CONTEXT_VARIABLE);
	...
	private CodeBlock generateInitializeCode() {
		...
		builder.addStatement("$T $L = $L.getDefaultListableBeanFactory()", DefaultListableBeanFactory.class,
				BEAN_FACTORY_VARIABLE, APPLICATION_CONTEXT_VARIABLE);
		builder.addStatement("$L.setAutowireCandidateResolver(new $T())", BEAN_FACTORY_VARIABLE,

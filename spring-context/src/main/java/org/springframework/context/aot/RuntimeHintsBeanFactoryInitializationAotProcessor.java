...
/**
 * AOT {@code BeanFactoryPostProcessor} that processes {@link RuntimeHintsRegistrar}
 * implementations declared as {@code spring.factories} or using
 * {@link ImportRuntimeHints @ImportRuntimeHints} annotated configuration classes or bean
 * methods.
 * <p>
 * This processor is registered by default in the {@link ApplicationContextAotGenerator}
 * as it is only useful in an AOT context.
 *
 * @author Brian Clozel
...
class RuntimeHintsBeanFactoryInitializationAotProcessor implements BeanFactoryInitializationAotProcessor {
	...
	public BeanFactoryInitializationAotContribution processAheadOfTime(ConfigurableListableBeanFactory beanFactory) {
		...
		List<RuntimeHintsRegistrar> registrars = new ArrayList<>(loader.load(RuntimeHintsRegistrar.class));
		for (String beanName : beanFactory.getBeanNamesForAnnotation(ImportRuntimeHints.class)) {
			ImportRuntimeHints annotation = beanFactory.findAnnotationOnBean(beanName, ImportRuntimeHints.class);
			if (annotation != null) {
	...
	static class RuntimeHintsRegistrarContribution implements BeanFactoryInitializationAotContribution {
		...
		public void applyTo(GenerationContext generationContext,
			...
			this.registrars.forEach(registrar -> {
				logger.trace(LogMessage.format("Processing RuntimeHints contribution from [%s]",

...
class CompileWithTargetClassAccessExtension implements InvocationInterceptor {
	...
	private void intercept(Invocation<Void> invocation, ExtensionContext extensionContext, Action action)
			throws Throwable {
	...
	private boolean isUsingForkedClassPathLoader(ExtensionContext extensionContext) {
		...
		return classLoader.getClass().getName().equals(CompileWithTargetClassAccessClassLoader.class.getName());
	}
	...
	private void runTestWithModifiedClassPath(ReflectiveInvocationContext<Method> invocationContext,
		...
		ClassLoader forkedClassPathClassLoader = new CompileWithTargetClassAccessClassLoader(testClass.getClassLoader(),
				targetClasses);
	...
	private String[] getTargetClasses(AnnotatedElement... elements) {
		...
		for (AnnotatedElement element : elements) {
			MergedAnnotation<?> annotation = MergedAnnotations.from(element).get(CompileWithTargetClassAccess.class);

...
class ConstantFieldFeature implements Feature {
	...
	private void duringSetup(DuringSetupAccessImpl access) {
		...
		try (DebugContext.Scope scope = debug.scope("ConstantFeature.duringSetup")) {
			...
			ConstantFieldSubstitutionProcessor substitutionProcessor = new ConstantFieldSubstitutionProcessor(debug,

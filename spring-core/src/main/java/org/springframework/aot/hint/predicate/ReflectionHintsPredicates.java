...
public class ReflectionHintsPredicates {
	...
	public static class ConstructorHintPredicate extends ExecutableHintPredicate<Constructor<?>> {
		...
		Predicate<RuntimeHints> exactMatch() {
			return hints -> {
				JavaReflectionHint hint = hints.reflection().getJavaReflectionHint(this.executable.getDeclaringClass());
				return (hint != null) && hint.constructors().anyMatch(executableHint -> {
					List<TypeReference> parameters = List.of(TypeReference.arrayOf(this.executable.getParameterTypes()));
					return includes(executableHint, "<init>", parameters, List.of(this.executableMode));
				});
			};
		}

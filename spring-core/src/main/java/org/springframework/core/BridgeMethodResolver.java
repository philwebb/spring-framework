...
public final class BridgeMethodResolver {
	...
	public static Method findBridgedMethod(Method bridgeMethod) {
		...
		List<Method> candidateMethods = null;
		Method[] methods = ReflectionUtils.getAllDeclaredMethods(bridgeMethod.getDeclaringClass());
		for (Method candidateMethod : methods) {
			if (isBridgedCandidateFor(candidateMethod, bridgeMethod)) {
				if (candidateMethods == null) {
					candidateMethods = new ArrayList<>(methods.length);
				}
	...
	private static boolean isResolvedTypeMatch(Method genericMethod, Method candidateMethod, Class<?> declaringClass) {
		...
		for (int i = 0; i < candidateParameters.length; i++) {
			ResolvableType genericParameter = ResolvableType.forMethodParameter(genericMethod, i, declaringClass, false);

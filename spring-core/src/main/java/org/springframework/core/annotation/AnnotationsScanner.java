...
abstract class AnnotationsScanner {
	...
	private static final String ENHANCED_CONFIGURATION_CLASS = "org.springframework.context.annotation.ConfigurationClassEnhancer$EnhancedConfiguration";
			...
			if (annotations.length != 0) {
				...
				for (int i = 0; i < annotations.length; i++) {
					if (isIgnorable(annotations[i].annotationType()) ||
							!AttributeValues.areValid(annotations[i])) {
						annotations[i] = null;
					}
					else {
						allIgnored = false;

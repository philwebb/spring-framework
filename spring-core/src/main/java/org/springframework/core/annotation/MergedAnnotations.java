...
interface MergedAnnotations extends Iterable<MergedAnnotation<?>> {
	...
	static MergedAnnotations from(AnnotatedElement element,
		...
		StandardDeclaredAnnotationsScanner annotations = new StandardDeclaredAnnotationsScanner(
				element, searchStrategy);
		return from(resolver, annotations);
	}
	 ...
	 * annotations
	 */
	static MergedAnnotations from(AnnotationTypeResolver resolver,
			Iterable<DeclaredAnnotations> annotations) {
		return new MappedAnnotations(resolver, annotations);
	}
	...
	public static enum SearchStrategy {
		...
		/**
		 * Search only directly declared annotation and meta-annotations without
		 * considering any inherited items.
		 */
		DIRECT,

		/**
		 * Fully Search all related elements, include those on any superclasses

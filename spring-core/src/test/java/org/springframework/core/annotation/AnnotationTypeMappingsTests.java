...
public class AnnotationTypeMappingsTests {
	...
	public void createWhenResolverIsNullShouldThrowException() {
		...
		new AnnotationTypeMappings(null, this.resolver.resolve(Example.class.getName()));
	}
	...
	public void createWhenSourceIsNullShouldThrowException() {
		...
		new AnnotationTypeMappings(this.resolver, null);
	}
	...
	public void getMetaAnnotationWhenHasOverrideReturnsNearest() {
		...
		AnnotationTypeMapping mappping = mappings.getMapping(Example.class.getName());
		assertThat(mappping.getAnnotationType().getClassName()).isEqualTo(
				Example.class.getName());
		assertThat(mappping.getAttributes().get("value")).isEqualTo("meta-override");
		assertThat(mappping.getDepth()).isEqualTo(1);
		assertThat(mappping.getParent().getAnnotationType().getClassName()).isEqualTo(
	...
	public void mapAttributeWhenHasDuplicateExplicitMirrorAttributesThrowsException() {
		...
		MappableAnnotation annotation = new MappableAnnotation(this.resolver, type,
				DeclaredAttributes.of("one", "1", "two", "2"));
		this.thrown.expect(AnnotationConfigurationException.class);
		this.thrown.expectMessage(
				"Different @AliasFor mirror values defined in attributes 'one' and 'two' on ["
	...
	public void getMappingWhenHasExplicitMirrorAttributesMapsEachMirror() {
		AnnotationType type = resolve(AliasForExplicitMirror.class);
		AnnotationTypeMappings mappings = getMappings(type);
		MappableAnnotation annotationOne = new MappableAnnotation(this.resolver, type,
		...
		MappableAnnotation annotationTwo = new MappableAnnotation(this.resolver, type,
	...
	public void getMappingWhenHasImplicitIndirectMirrorAttributesMapsParents() {
		...
		assertThat(mappedImplicit.getString("three")).isEqualTo("x");
		MergedAnnotation<?> mapped = mappings.getMapping(Example.class.getName()).map(
				annotation, false);
		assertThat(mapped.getString("value")).isEqualTo("1");
	}
	...
	private AnnotationTypeMappings getMappings(String type) {
		return new AnnotationTypeMappings(this.resolver, this.resolver.resolve(type));
	}

	private AnnotationTypeMappings getMappings(AnnotationType type) {
		return new AnnotationTypeMappings(this.resolver, type);
	}

...
/**
 * Tests for {@link MergedAnnotations} and {@link MergedAnnotation} based on the
 * original tests for {@code AnnotatedElementUtils} and {@code AnnotationUtils}.
 *
 * @author Phillip Webb
 */
public class MergedAnnotationsBackCompatibiltyTests {
	...
	public void streamWhenNotAnnotatedReturnsEmpty() {
		...
		assertThat(annotations.stream()).isEmpty();
	}

	@Test
	public void streamFromAnnotationSourceWhenHasMetaDepth1ReturnsMetaAnnotations() {
		MergedAnnotations annotations = MergedAnnotations.from(
				TransactionalComponent.class);
		assertThat(annotations.stream().map(MergedAnnotation::getType)).containsExactly(
				Transactional.class.getName(), Component.class.getName(),
				Indexed.class.getName());
	}

	@Test
	public void streamFromAnnotationSourceWhenHasMetaDepth2ReturnsMetaAnnotations() {
		MergedAnnotations annotations = MergedAnnotations.from(
				SearchStrategy.INHERITED_ANNOTATIONS,
				ComposedTransactionalComponent.class);
		assertThat(annotations.stream().map(MergedAnnotation::getType)).containsExactly(
				TransactionalComponent.class.getName(), Transactional.class.getName(),
				Component.class.getName(), Indexed.class.getName());
	}

	@Test
	public void streamWhenNotAnnotatedFindsNoMetaPresentAnnotations() {
		MergedAnnotations annotations = MergedAnnotations.from(NonAnnotatedClass.class);
		assertThat(annotations.stream(Transactional.class)).noneMatch(
				MergedAnnotation::isMetaPresent);
	}

	@Test
	public void streamWhenHasMetaDepth0FindsNoMetaPresentAnnotations() {
		MergedAnnotations annotations = MergedAnnotations.from(TransactionalComponentClass.class);
		assertThat(annotations.stream(TransactionalComponent.class)).noneMatch(
				MergedAnnotation::isMetaPresent);
	}

	@Test
	public void streamWhenHasMetaDepth1FindsMetaPresentAnnotations() {
		MergedAnnotations annotations = MergedAnnotations.from(TransactionalComponentClass.class);
		assertThat(annotations.stream(Transactional.class)).anyMatch(
				MergedAnnotation::isMetaPresent);
		assertThat(annotations.stream(Component.class)).anyMatch(
				MergedAnnotation::isMetaPresent);
	}

	@Test
	public void streamWhenHasMetaDepth2FindsMetaPresentAnnotations() {
		MergedAnnotations annotations = MergedAnnotations.from(ComposedTransactionalComponentClass.class);
		assertThat(annotations.stream(Transactional.class)).anyMatch(
				MergedAnnotation::isMetaPresent);
		assertThat(annotations.stream(Component.class)).anyMatch(
				MergedAnnotation::isMetaPresent);
		assertThat(annotations.stream(ComposedTransactionalComponent.class)).noneMatch(
				MergedAnnotation::isMetaPresent);
	}

	@Test
	public void isPresentWhenNotAnnotatedReturnsFalse() {
		MergedAnnotations annotations = MergedAnnotations.from(NonAnnotatedClass.class);
		assertThat(annotations.isPresent(TransactionalComponent.class)).isFalse();
		assertThat(annotations.isPresent(TransactionalComponent.class.getName())).isFalse();
		assertThat(annotations.get(TransactionalComponent.class).isPresent()).isFalse();
		assertThat(annotations.get(TransactionalComponent.class.getName()).isPresent()).isFalse();
	}

	@Test
	public void isPresentWhenHasMetaDepth0ReturnsTrue() {
		MergedAnnotations annotations = MergedAnnotations.from(TransactionalComponentClass.class);
		assertThat(annotations.isPresent(TransactionalComponent.class)).isTrue();
	}

	@Test
	public void isPresentWhenHasSubclassWithMetaDepth0ButSearchDirectReturnsFalse() {
		MergedAnnotations annotations = MergedAnnotations.from(SubTransactionalComponentClass.class);
		assertThat(annotations.isPresent(TransactionalComponent.class)).isFalse();
	}

	@Test
	public void isPresentWhenHasMetaDepth1ReturnsTrue() {
		MergedAnnotations annotations = MergedAnnotations.from(TransactionalComponentClass.class);
		assertThat(annotations.isPresent(Component.class)).isTrue();
	}

	@Test
	public void isPresentWhenHasMetaDepth2ReturnsTrue() {
		MergedAnnotations annotations = MergedAnnotations.from(ComposedTransactionalComponentClass.class);
	...
	@Test
	public void asMapWhenNotAnnotatedReturnsEmptyMap() {
		MergedAnnotations annotations = MergedAnnotations.from(NonAnnotatedClass.class);
		assertThat(annotations.get(Transactional.class).asMap()).isEmpty();
	}

	@Test
	public void asMapWhenHasLocalAnnotationReturnsMap() {
		MergedAnnotations annotations = MergedAnnotations.from(TxConfig.class);
		assertThat(annotations.get(Transactional.class).asMap().get("value")).isEqualTo("TxConfig");
	}
	...
	public void getAllAnnotationAttributesOnClassWithLocalComposedAnnotationAndInheritedAnnotation() {
		MultiValueMap<String, Object> attributes = getAllAnnotationAttributes(SubClassWithInheritedAnnotation.class, Transactional.class.getName());
	...
	public void getAllAnnotationAttributesFavorsInheritedAnnotationsOverMoreLocallyDeclaredComposedAnnotations() {
		MultiValueMap<String, Object> attributes = getAllAnnotationAttributes(SubSubClassWithInheritedAnnotation.class, Transactional.class.getName());
	...
	public void getAllAnnotationAttributesFavorsInheritedComposedAnnotationsOverMoreLocallyDeclaredComposedAnnotations() {
		MultiValueMap<String, Object> attributes = getAllAnnotationAttributes( SubSubClassWithInheritedComposedAnnotation.class, Transactional.class.getName());
	...
	public void getAllAnnotationAttributesOnClassWithMultipleComposedAnnotations() {
		MultiValueMap<String, Object> attributes = getAllAnnotationAttributes(TxFromMultipleComposedAnnotations.class, Transactional.class.getName());
	...
	public void getMergedAnnotationAttributesOnMetaCycleAnnotatedClassWithMissingTargetMetaAnnotation() {
		AnnotationAttributes attributes = getMergedAnnotationAttributes(MetaCycleAnnotatedClass.class, Transactional.class.getName());

...
/**
 * Tests for {@link StandardDeclaredAnnotationsScanner}.
 *
 * @author Phillip Webb
 */
public class StandardDeclaredAnnotationsScannerTests {
	...
	private Stream<String> scan(AnnotatedElement source, SearchStrategy searchStrategy) {
		StandardDeclaredAnnotationsScanner scanner = new StandardDeclaredAnnotationsScanner(
				source, searchStrategy);
		return stream(scanner).flatMap(new Mapper());
	}

	private Stream<DeclaredAnnotations> stream(
			StandardDeclaredAnnotationsScanner scanner) {

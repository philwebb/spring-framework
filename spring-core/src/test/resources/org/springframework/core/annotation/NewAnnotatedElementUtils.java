 ...
 */
public abstract class NewAnnotatedElementUtils {
	 ...
	 * typically for use with other methods on {@link NewAnnotatedElementUtils}.
	 ...
	 * {@linkplain NewAnnotatedElementUtils class-level javadoc}.
	...
	private static Set<String> getMetaAnnotationTypes(AnnotatedElement element, @Nullable Annotation composed) {
		...
		MergedAnnotation<?> parent = mergedAnnotations.get(composed.annotationType().getName());
		Set<String> result = mergedAnnotations.stream().filter(parent::isParentOf).map(

...
class PersistenceAnnotationBeanPostProcessorAotContributionTests {
	...
	void processAheadOfTimeWhenPersistenceUnitOnPublicField() {
		...
		testCompile(registeredBean, (actual, compiled) -> {
			...
			this.beanFactory.registerSingleton("entityManagerFactory", entityManagerFactory);
	...
	void processAheadOfTimeWhenCustomPersistenceUnitOnPublicSetter() {
		RegisteredBean registeredBean = registerBean(CustomUnitNamePublicPersistenceUnitMethod.class);
		testCompile(registeredBean, (actual, compiled) -> {
			EntityManagerFactory entityManagerFactory = mock(EntityManagerFactory.class);
	...
	void processAheadOfTimeWhenPersistenceContextOnPrivateField() {
		RegisteredBean registeredBean = registerBean(DefaultPersistenceContextField.class);
		testCompile(registeredBean, (actual, compiled) -> {
			EntityManagerFactory entityManagerFactory = mock(EntityManagerFactory.class);
			this.beanFactory.registerSingleton("entityManagerFactory", entityManagerFactory);
			...
			assertThat(this.generationContext.getRuntimeHints().reflection().typeHints()).singleElement()
					.satisfies(typeHint -> {
						assertThat(typeHint.getType())
								.isEqualTo(TypeReference.of(DefaultPersistenceContextField.class));
	...
	void processAheadOfTimeWhenPersistenceContextWithCustomPropertiesOnMethod() {
		RegisteredBean registeredBean = registerBean(CustomPropertiesPersistenceContextMethod.class);
		testCompile(registeredBean, (actual, compiled) -> {
			EntityManagerFactory entityManagerFactory = mock(EntityManagerFactory.class);
			this.beanFactory.registerSingleton("entityManagerFactory", entityManagerFactory);
			CustomPropertiesPersistenceContextMethod instance = new CustomPropertiesPersistenceContextMethod();
			actual.accept(registeredBean, instance);
			Field field = ReflectionUtils.findField(CustomPropertiesPersistenceContextMethod.class, "entityManager");
			ReflectionUtils.makeAccessible(field);
			EntityManager sharedEntityManager = (EntityManager) ReflectionUtils.getField(field, instance);
			InvocationHandler invocationHandler = Proxy.getInvocationHandler(sharedEntityManager);
			assertThat(invocationHandler).extracting("properties").asInstanceOf(InstanceOfAssertFactories.MAP)
					.containsEntry("jpa.test", "value").containsEntry("jpa.test2", "value2");
			assertThat(this.generationContext.getRuntimeHints().reflection().typeHints()).isEmpty();
		});
	}
	...
	private void testCompile(RegisteredBean registeredBean,
		...
		BeanRegistrationAotContribution contribution = postProcessor.processAheadOfTime(registeredBean);
	...
	static class CustomPropertiesPersistenceContextMethod {
		...
		@PersistenceContext(properties = { @PersistenceProperty(name = "jpa.test", value = "value"),

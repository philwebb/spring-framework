...
public class JibxMarshallerTests extends AbstractMarshallerTests<JibxMarshaller> {
	...
	public void indentation() throws Exception {
		this.marshaller.setIndent(4);
		StringWriter writer = new StringWriter();
		this.marshaller.marshal(this.flights, new StreamResult(writer));
		String expected =
	...
	public void encodingAndStandalone() throws Exception {
		...
		this.marshaller.setStandalone(Boolean.TRUE);
		StringWriter writer = new StringWriter();
		this.marshaller.marshal(this.flights, new StreamResult(writer));
	...
	public void dtd() throws Exception {
		this.marshaller.setDocTypeRootElementName("flights");
		this.marshaller.setDocTypeSystemId("flights.dtd");
		StringWriter writer = new StringWriter();
		this.marshaller.marshal(this.flights, new StreamResult(writer));
	...
	public void supports() throws Exception {
		assertTrue("JibxMarshaller does not support Flights", this.marshaller.supports(Flights.class));
		assertTrue("JibxMarshaller does not support FlightType", this.marshaller.supports(FlightType.class));
		assertFalse("JibxMarshaller supports illegal type", this.marshaller.supports(getClass()));
	}

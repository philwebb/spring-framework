...
public class ContextCacheTestUtils {
	...
	public static final void assertContextCacheStatistics(ContextCache contextCache, String usageScenario,
		...
		assertThat((long) contextCache.size()).as("Verifying number of contexts in cache (" + usageScenario + ").").isEqualTo((long) expectedSize);
		assertThat((long) contextCache.getHitCount()).as("Verifying number of cache hits (" + usageScenario + ").").isEqualTo((long) expectedHitCount);
		assertThat((long) contextCache.getMissCount()).as("Verifying number of cache misses (" + usageScenario + ").").isEqualTo((long) expectedMissCount);
	}

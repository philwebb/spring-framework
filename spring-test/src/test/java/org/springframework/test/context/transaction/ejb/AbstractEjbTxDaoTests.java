...
public abstract class AbstractEjbTxDaoTests extends AbstractTransactionalJUnit4SpringContextTests {
	...
	public void test1InitialState() {
		...
		assertThat((long) count).as("New TestEntity should have count=0.").isEqualTo((long) 0);
	}

	@Test
	public void test2IncrementCount1() {
		int count = dao.incrementCount(TEST_NAME);
		assertThat((long) count).as("Expected count=1 after first increment.").isEqualTo((long) 1);
	}
	...
	public void test3IncrementCount2() {
		int count = dao.getCount(TEST_NAME);
		assertThat((long) count).as("Expected count=1 after test2IncrementCount1().").isEqualTo((long) 1);

		count = dao.incrementCount(TEST_NAME);
		assertThat((long) count).as("Expected count=2 now.").isEqualTo((long) 2);
	}

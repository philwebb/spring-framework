...
class ImportHttpServiceClientsRegistrar implements ImportBeanDefinitionRegistrar {
	...
	private void register(BeanDefinitionRegistry registry, BeanNameGenerator defaultNameGenerator,
		...
		Using using = new Using(annotation.getString("using"));
		BeanNameGenerator beanNameGenerator = getBeanNameGenerator(annotation, defaultNameGenerator);
		Scanner scanner = new Scanner(registry, this.environment, this.resourceLoader, using);
		...
		scanner.setProxyFactory(using::instanceSupplier);
		for (String client : annotation.getStringArray("clients")) {
			registerClient(registry, scanner, using, client, beanNameGenerator);
		}
	...
	private void registerClient(BeanDefinitionRegistry registry, Scanner scanner, Using using, String client,
	 ...
	 */
	private record Using(String id) {

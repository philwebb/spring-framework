...
/**
 * {@link InstanceSupplier} that uses a {@link HttpServiceProxyFactory} obtained directly
 * or via a {@link HttpServiceProxyFactoryProvider} to create the bean instance.
 *
 * @author Phillip Webb
 * @since 7.0
...
class HttpServiceProxyClientInstanceSupplier<T> implements InstanceSupplier<T> {
	...
	HttpServiceProxyClientInstanceSupplier(String serviceType, @Nullable String group) {
	...
	private HttpServiceProxyFactory getFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		HttpServiceProxyFactory factory = provider.getHttpServiceProxyFactory(this.group);
		if (factory == null) {
			throw new NoSuchElementException("Group '%s' is not available".formatted(this.group));
		}
		return factory;
	}

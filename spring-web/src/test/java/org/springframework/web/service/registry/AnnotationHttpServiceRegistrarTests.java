...
public class AnnotationHttpServiceRegistrarTests {
	...
	private static class TestGroupRegistry implements AbstractHttpServiceRegistrar.GroupRegistry {
		...
		private record TestGroupSpec(Map<String, StubGroup> groupMap, Function<Class<?>, String> nameProvider,
			...
			private StubGroup getOrCreateGroup() {
				String groupName = nameProvider().apply(Object.class);
				ClientType clientType = clientTypeProvider().apply(Object.class);
				return groupMap.computeIfAbsent(groupName, name -> new StubGroup(name, clientType));
			}
		}
	}
